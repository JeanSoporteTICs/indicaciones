...
// --- Validar m©todo y payload ---
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    fail('M©todo no permitido (use POST)', 405);
}

$raw = $_POST['payload'] ?? '';
if (!$raw) {
    fail('Payload vac­o o no enviado', 400);
}

$data = json_decode($raw, true);
if (!is_array($data)) {
    fail('JSON inv¡lido en payload', 400);
}

// --- Autoload Composer ---
$autoload = __DIR__ . '/vendor/autoload.php';
if (!file_exists($autoload)) {
    fail('No se encontr³ vendor/autoload.php. Ejecuta composer install.', 500);
}
require $autoload;
...
if (!file_exists($templatePath)) {
    fail('No se encontr³ la plantilla en templates/11.11.25.xlsx', 500);
}
...
/**
 * Rellena un marcador ºnico (si existe).
 */
...
$ws = null;
foreach ($indicacionesSheets as $sheetName) {
...
/* ---- Datos antropom©tricos y c¡lculos ---- */
...
$marca = function ($v) {
    // Devuelve "X" si es true/1/"on", vac­o si no
    if ($v === true)  return 'X';
    if ($v === 1)     return 'X';
    if ($v === '1')   return 'X';
    if ($v === 'on')  return 'X';
...
try {
    ...
    // No forzar rec¡lculo de f³rmulas aqu­ (lo har¡ Excel al abrir)
...
if ($size === false || $size === 0) {
    @unlink($tmpFile);
    fail('El archivo generado est¡ vac­o o no se pudo leer.', 500);
}
